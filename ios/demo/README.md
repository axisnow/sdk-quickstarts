# AgentSDK 快速接入：iOS ObjectiveC

[English](./README.en.md) | 简体中文

本快速接入指南面向使用 ObjectiveC 编写、通过 TCP / Socket 层发起 API 请求的原生 iOS 应用，帮助你为这些请求接入 AgentSDK 的保护能力。如果你的场景与此不符，请查阅更贴近的接入指南。
...
1. `AXSecurity.framework` — AgentSDK 核心 SDK
...
在使用 `AXService` 前，必须在 App 启动时完成初始化，通常放在 `application:didFinishLaunchingWithOptions:` 中。`AXService` 为实例化对象 —— 建议保留一个长生命周期的实例并在整个 App 中复用：
...
| `AXConfig.edgeAddresses` | Edge 节点地址数组（必填），`NSArray<NSString *>`，至少 1 个，推荐 2+ |
...
```objc
AXLocalProxy http;
[self.axservice getLocalHTTPProxy:&http];    // 本地 HTTP 代理端点

AXLocalProxy socks5;
[self.axservice getLocalSocks5Proxy:&socks5]; // 本地 SOCKS5 代理端点
```
...
[self.axservice clearDNSCache]; // 需要时清除缓存
```
