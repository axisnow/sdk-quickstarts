...
本快速接入文档面向使用 Java 开发、并通过 OkHttp 发起需要 AgentSDK 保护的 API 调用的原生 Android 应用。如果你的场景不符，请查看其他更适合的快速接入文档。

本页提供了将 AgentSDK 集成到你的应用中的全部步骤。此外，我们还提供了基于 [java-okhttp-demo](java-okhttp-demo/) 的分步教程示例。
...
该方法返回的 `OkHttpClient` 已自动配置 AgentSDK 本地代理并被缓存。所有需要保护的 API 调用都应使用这个 client。

## 自定义 OkHttp Builder

默认情况下，`getOkHttpClient()` 使用 `new OkHttpClient.Builder()` 构造 client。但你现有的代码可能使用了自定义 client，例如不同的超时时间或其他拦截器。假设你原有代码如下：
