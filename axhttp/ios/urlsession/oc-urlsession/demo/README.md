# AgentSDK 快速接入：iOS ObjectiveC NSURLSession

[English](./README.en.md) | 简体中文

本快速接入指南面向使用 ObjectiveC 编写、通过 `NSURLSession` 发起 API 请求的原生 iOS 应用，帮助你为这些请求接入 AgentSDK 的保护能力。如果你的场景与此不符，请查阅更贴近的接入指南。
...
1. `AXSecurityNSURLSession.framework` — AgentSDK NSURLSession 包装层
...
## 初始化 AXNSURLSessionService

在创建任何 `AXNSURLSession` 之前，必须在 App 启动时初始化 `AXNSURLSessionService`，通常放在 `application:didFinishLaunchingWithOptions:` 中：
...
- (BOOL)application:(UIApplication *)application
    ...
    config.edgeAddresses   = @[ @"Edge 节点 IP 或域名" ];

    int r = [AXNSURLSessionService Initialize:config];
    if (r != 0) {
        NSLog(@"AgentSDK 初始化失败: %d", r);
    }
    return YES;
}
```
...
> **重要：** `Initialize:` 必须在构造任何 `AXNSURLSession` 之前调用且仅调用一次。不支持多次调用。
...
| `AXConfig.edgeAddresses` | Edge 节点地址列表（必填），`NSArray<NSString *>`，至少 1 个，推荐 2+ |

本 demo 仅使用上述最小必填字段。`AXConfig` 的完整选项（DNS 配置、加密隧道开关等）以及参数语义详见 AgentSDK 接入指南。

## 使用 AXNSURLSessionService
...
返回的对象是一个 `NSURLSession`，其所有流量会透明地经由 AgentSDK 本地代理转发。像使用普通 `NSURLSession` 一样，让它与依赖它的业务功能保持相同的生命周期即可。
...
若 SDK 无法连接 Edge 节点或凭证非法，`Initialize:` 会返回非零状态码。请在构造 session 之前务必检查：
...
1. `AXNSURLSessionService Initialize:` 未输出错误日志。
...
| 现象 | 原因 | 处理方式 |
|------|------|---------|
| `Initialize:` 返回负数 | 凭证无效或 Edge 不可达 | 核对 `accessKeyId`、`accessKeySecret`、`edgeAddresses` |
| 控制台出现 `[AXNSURLSession] proxy lookup failed` | `AXNSURLSession` 在 `Initialize:` 成功前被构造 | 确认 `Initialize:` 返回 `0` 后再调用 `+sessionWithConfiguration:` |
| 请求超时或报错 `NSURLErrorCannotConnectToHost` | 本地代理未运行 | 检查 `Initialize:` 返回码及 Edge 连通性 |
...
5. **在 `Initialize:` 返回 `0` 之前构造 `AXNSURLSession`** — session 构造时会读取当时的本地代理地址。如果在 `AXNSURLSessionService Initialize:` 成功之前构造 `AXNSURLSession`，底层 session 可能因缺少代理地址而无法路由。请务必先完成初始化。
